...
def _sync_catalog(B):
    for key, labels in CATALOG.items():
        B.db.conn.execute("INSERT OR IGNORE INTO settings(key,value) VALUES(?,?)", ("service_active_" + key, "1"))
        for lang, text in labels.items():
            B.db.conn.execute("INSERT OR IGNORE INTO settings(key,value) VALUES(?,?)", ("service_label_{}_{}".format(key, lang), text))
        B.db.conn.execute("INSERT OR IGNORE INTO services(key,name,description,price,active) VALUES(?,?,?,?,1)", (key, labels["fa"], "", int(B.db.setting("service_price_" + key, B.db.setting("price_" + key, "0")) or 0)))
    B.db.conn.commit()
...
def install():
    ...
    if getattr(B, "_production_final_installed", False): return
    ...
    def remember(uid, label):
        ...
        try:
            B.db.conn.execute("INSERT OR REPLACE INTO ui_callbacks(token,user_id,label,created_at) VALUES(?,?,?,?)", (token, str(uid), str(label), B.now())); B.db.conn.commit()
        except Exception: log.exception("callback persistence failed")
        return token
    F._remember = remember
    async def persistent_ui_callback(update, context):
        q = update.callback_query; await q.answer(); token = str(q.data or ""); row = None
        try: row = B.db.conn.execute("SELECT label FROM ui_callbacks WHERE token=?", (token,)).fetchone()
        except Exception: log.exception("callback lookup failed")
        if not row:
        ...
        label = str(row["label"]); fake = F._fake_update(update, label)
        try:
            result = await B.router(fake, context)
            if result is None:
                result = await B.ptext(fake, context)
                if result is None: await B.service_text(fake, context)
        except Exception:
    ...
    async def router(update, context):
        ...
        if B.admin(uid):
            ...
            if mode == "services":
                if text == "⬅️ منوی مدیریت": st["production_admin_mode"] = None; return await update.message.reply_text("🛠 پنل مدیریت کامل", reply_markup=B.amenu())
                key = next((k for k in CATALOG if k in text), None)
                if key:
                    st["production_service_key"] = key
                    return await update.message.reply_text("🔧 {}\n📌 وضعیت: {}\n💰 قیمت: {:,} تومان".format(_label(B, key, "fa"), "باز" if _active(B, key) else "بسته", _price(B, key)), reply_markup=B.kb([["🔄 باز/بسته"], ["✏️ متن فارسی", "✏️ English"], ["✏️ العربية", "💰 قیمت"], ["⬅️ بازگشت"]]))
            if mode == "service_action":
                key = st.get("production_service_key")
                if key:
                    if text == "🔄 باز/بسته":
                        new = "0" if _active(B, key) else "1"; B.db.set_setting("service_active_" + key, new); return await update.message.reply_text("🟢 خدمت باز شد." if new == "1" else "🔴 خدمت بسته شد.", reply_markup=B.kb(_service_admin_rows(B)))
                    if text == "💰 قیمت": st["production_admin_mode"] = "service_price"; return await update.message.reply_text("💰 قیمت جدید را فقط به تومان وارد کنید:", reply_markup=B.kb([["⬅️ بازگشت"]]))
                    lang = {"✏️ متن فارسی":"fa", "✏️ English":"en", "✏️ العربية":"ar"}.get(text)
                    if lang: st["production_label_lang"] = lang; st["production_admin_mode"] = "service_label"; return await update.message.reply_text("✏️ متن جدید این بخش را ارسال کنید:", reply_markup=B.kb([["⬅️ بازگشت"]]))
            if mode == "service_label" and st.get("production_service_key"):
                key = st["production_service_key"]; lang = st.get("production_label_lang", "fa"); B.db.set_setting("service_label_{}_{}".format(key, lang), text); st["production_admin_mode"] = "services"; return await update.message.reply_text("✅ متن {} ذخیره شد.".format(lang), reply_markup=B.kb(_service_admin_rows(B)))
            if mode == "service_price" and st.get("production_service_key"):
                ...
                key = st["production_service_key"]; amount = int(raw); B.db.set_setting("service_price_" + key, amount); B.db.set_setting("price_" + key, amount); B.db.conn.execute("UPDATE services SET price=? WHERE key=?", (amount, key)); B.db.conn.commit(); st["production_admin_mode"] = "services"; return await update.message.reply_text("✅ قیمت به {:,} تومان تغییر کرد.".format(amount), reply_markup=B.kb(_service_admin_rows(B)))
    ...
    B.router = router; B._production_final_installed = True; log.info("production final patch installed")
