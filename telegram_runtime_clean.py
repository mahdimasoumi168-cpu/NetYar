...
async def _start(update, context):
    ...
    # Preserve durable partner/admin identity while resetting only transient flow state.
    old = B.S.get(uid, {})
...
def _install_features(app):
    # These are the only production feature layers installed by the canonical runtime.
    try:
        import telegram_business_features as F
        F.install(app, B)
    except Exception:
        log.exception("Telegram business features failed to install")
        raise
    try:
        import telegram_admin_plus as A
        # admin_plus owns the full admin inline panel and price/credit controls.
        ...
        log.exception("Telegram admin panel failed to install")
        raise
    try:
        import partner_pricing as P
        P.install_telegram(app, B)
    except Exception:
        log.exception("Partner pricing failed to install")
        raise
...
def build():
    ...
    # Legacy bot handlers remain as the final fallback for existing services.
    ...
    log.info("Canonical Telegram handlers installed with persistent UI, admin, pricing and ticket features")
    return app
