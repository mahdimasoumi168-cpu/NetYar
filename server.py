...
async def telegram_update(request:Request):
    ...
    try:
        ...
        await telegram_app.update_queue.put(update); log.info("Telegram update accepted: update_id=%s",payload.get("update_id")); return {"ok":True}
...
def _normalize_rubika_button(update,rb):
    ...
    maps={
      ...
      "print_color":{"1":"⚫ سیاه و سفید","2":"🌈 رنگی","0":rb.CANCEL},"print_side":{"1":"📄 یک‌رو","2":"🔄 پشت‌ورو","0":rb.CANCEL},"print_files":{"1":"✅ تأیید","0":rb.CANCEL},"print":{"1":"✅ تأیید","0":rb.CANCEL},
...
async def _run_rubika(update,rb):
    try:
        normalized=_normalize_rubika_button(update,rb);await asyncio.to_thread(rb.process,normalized)
        inner=_rubika_inner(update);log.info("Rubika update processed: type=%s text=%s",inner.get("type") if isinstance(inner,dict) else None,_rubika_text(update))
