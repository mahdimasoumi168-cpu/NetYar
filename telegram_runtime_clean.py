...
async def _safe_call(fn,update,context,*extra):
    try:
        result=fn(update,context,*extra); return await result if inspect.isawaitable(result) else result
...
def _install_features(app):
    for module,fn,args in (("telegram_offhours_partner_gate_v2","install",(app,B)),("telegram_offhours_absolute_start_guard","install",(app,B)),("telegram_startup_button_firewall","install",(app,B)),("telegram_business_features","install",(app,B)),("telegram_ui_policy_v2","install",(app,B)),("telegram_partner_ui_fix","install",(app,B)),("telegram_public_tracking","install",(app,B)),("telegram_service_billing_v3_fix","install",(app,B)),("telegram_sim_service_v2","install",(app,B)),("telegram_irancell_partner_service","install",(app,B))):
        try:
    ...
    try:
        import telegram_language_consistency as TLC; result=TLC.install(B)
        if inspect.isawaitable(result):asyncio.run(result)
    except Exception:log.exception("language consistency unavailable")
    try:
        import telegram_topup_invoice as TI; result=TI.install(B)
        if inspect.isawaitable(result):asyncio.run(result)
    ...
    for module,label in (("telegram_partner_runtime_fix_v31","partner runtime fix v31"),("telegram_partner_final_router_v29","partner router v29"),("telegram_absolute_callback_hardening_v30","absolute callback hardening v30"),("telegram_management_only_v32","management-only partner UI v32"),("telegram_session_and_context_hardening_v33","session/context hardening v33"),("telegram_final_admin_navigation_v3","final admin navigation v3"),("telegram_partner_navigation_final_v33","partner navigation v33"),("telegram_final_iranian_menu_v38","Iranian menu v38"),("telegram_final_request_partner_guard_v1","final request/partner routing guard v1"),("telegram_security_code_image_flow_v4","security-code image workflow v4")):
        try:
            m=__import__(module); result=m.install(app,B)
            if inspect.isawaitable(result):asyncio.run(result)
...
def _self_check():
    required=("main","partner","fida","gov","prt","ptrack","phistory","media","router","admin","cancel"); missing=[name for name in required if not callable(getattr(B,name,None))]
...
def build():
    ...
    app=Application.builder().token(token).build(); _self_check(); app.add_handler(CommandHandler("addpartner",B.addpartner),group=-100); app.add_handler(MessageHandler(filters.Regex(r"^/Admin2025$"),B.admin_command),group=-100); app.add_handler(CallbackQueryHandler(B.admin_cb,pattern=r"^(tu|pay|req|admin):"),group=0); app.add_handler(MessageHandler(filters.PHOTO | filters.Document.ALL,B.media),group=10); app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND,B.router),group=20); _install_features(app); B.start=_start; log.info("Canonical Telegram Application built successfully; Persian-only startup active"); return app
