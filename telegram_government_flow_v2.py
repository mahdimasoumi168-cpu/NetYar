...
Document types: Amayesh card, temporary card, passport, residence booklet.
Family code is requested only for Amayesh. Passport number is replaced by
residence-booklet number for the residence-booklet path. Postal code is
collected at the end. Service payment is handled by an invoice link; payment
is never considered confirmed merely because the link was opened.
"""
import re
...
def doc_menu():return InlineKeyboardMarkup([[InlineKeyboardButton("🪪 کارت آمایش",callback_data="govv2:card"),InlineKeyboardButton("🪪 کارت موقت",callback_data="govv2:temporary")],[InlineKeyboardButton("🛂 گذرنامه",callback_data="govv2:passport"),InlineKeyboardButton("📗 دفترچه اقامت",callback_data="govv2:residence")],[InlineKeyboardButton("❌ انصراف",callback_data="govv2:cancel")]])
...
def install(app,B):
 ...
 async def cb(update,context):
  ...
  if d[1]=="cancel":
   ...
   if rid:
    try:
     B.db.conn.execute("UPDATE requests SET status='cancelled',updated_at=? WHERE id=? AND payment_status!='paid'",(B.now(),int(rid)));B.db.conn.commit()
  ...
  st["gov_doc_type"]={"card":"card","temporary":"temporary_card","passport":"passport","residence":"residence_booklet"}[d[1]];st["mode"]="govv2_phone";return await q.message.reply_text("📱 شماره موبایل مشترک را وارد کنید:",reply_markup=cancel())
 async def text(update,context):
  ...
  if mode=="govv2_special":
   ...
   st["mode"]="govv2_identity_number";return await update.message.reply_text("🛂 شماره گذرنامه مشترک را وارد کنید:" if st.get("gov_doc_type")=="passport" else "📗 شماره دفترچه اقامت مشترک را وارد کنید:" if st.get("gov_doc_type")=="residence_booklet" else "🪪 شماره کارت موقت مشترک را وارد کنید:",reply_markup=cancel())
 ...
 async def media(update,context):
  ...
  if st.get("gov_doc_type")=="temporary_card":fields.append(("temporary_card_number",st.get("gov_identity_number")))
  ...
  return await msg.reply_text(invoice_text("فاکتور خدمات حل مشکل سامانه دولت من",amount,code,B),reply_markup=invoice_markup(B),parse_mode="HTML")
 ...
 app.add_handler(MessageHandler(filters.PHOTO|filters.Document.ALL,receipt),group=-68)
 app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND,lambda u,c:text(u,c,B)),group=-69)
 app.add_handler(MessageHandler(filters.PHOTO|filters.Document.ALL,lambda u,c:media(u,c,B)),group=-67)
 B._gov_v2=True
