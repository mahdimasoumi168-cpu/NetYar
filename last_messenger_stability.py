...
ADMIN_LABELS = {
    "🛠 پنل مدیریت بات", "🛠 پنل مدیریت", "👤 کاربران", "👤 پنل کاربران", "👥 همکاران", "🤝 همکاران",
    "➕ افزودن همکار", "💰 شارژها", "💰 پرداخت‌های مشتری", "📋 درخواست‌ها", "⚙️ قیمت‌ها", "🟢 خدمات",
    "📊 گزارش کامل", "📊 گزارش", "📣 اعلان همگانی", "🤖 بات‌های متصل", "➕ افزودن بات", "🧾 لاگ مدیریت",
    "⚙️ تنظیمات سیستم", "⚙️ تنظیمات پایه", "🎫 تیکت‌ها", "🟢/🔴 خدمات ایرانی", "🟢/🔴 خدمات اتباع",
    "💰 قیمت خدمات", "📝 تغییر متن‌ها", "📎 مدارک و فایل‌ها", "👤 مدیران", "🤖 پیام‌رسان‌ها", "📊 گزارش‌ها",
    "📞 پشتیبانی", "⬅️ منوی اصلی", "⬅️ بازگشت",
}


def _callback_update(q, label=""):
    class Msg:
        def __init__(self, original):
            self._original = original
            self.chat = getattr(original, "chat", None)
            self.from_user = q.from_user
            self.text = label or getattr(original, "text", "")
...
def _visible_label(q):
    try:
        ...
        log.exception("callback visible-label recovery failed")
    return ""
    ...
    try:
        ...
        if not getattr(B, "_last_messenger_telegram_guard", False):
            ...
            async def ik_guard(update, context):
                ...
                if not label:
                    # A deploy/restart can clear the process-local action map.
                    # The button still contains its visible text, so recover it
                    # instead of telling the user that a valid button expired.
                    label = _visible_label(q)
                await q.answer()
                if not label:
                    return await q.message.reply_text("🔄 منو به‌روزرسانی شد. لطفاً یکی از گزینه‌های فعلی را انتخاب کنید.", reply_markup=B.main(q.from_user.id))
                ...
                if B.admin(q.from_user.id) and label in ADMIN_LABELS:
                    ...
                    if handler is not None:
                        return await handler(_callback_update(q, label), context)

                normalized = label.strip()
                if normalized in {"👥 پنل همکاران", "🔵 👥 پنل همکاران", "🔵 👥 Partner panel", "🔵 👥 لوحة الشركاء"}:
                    try:
                        await P._remove_old_telegram_keyboard(q.message)
                    except Exception:
                        pass
                    return await B.partner(_callback_update(q, "👥 پنل همکاران"), context)

                if normalized in {getattr(B, "CANCEL", "❌ انصراف"), "❌ لغو", "Cancel", "إلغاء"}:
                    return await B.cancel(_callback_update(q, "❌ انصراف"), context)

                # Recovered labels are routed through the canonical text router,
                # so a restarted process does not need the old callback token.
                if label and not actions.get(str(q.data or "")):
                    try:
                        return await B.router(_callback_update(q, label), context)
                    except Exception:
                        log.exception("recovered Telegram button failed: %s", label)
                        return await q.message.reply_text("❌ اجرای گزینه با خطا مواجه شد. لطفاً دوباره انتخاب کنید.", reply_markup=B.main(q.from_user.id))
            ...
            log.info("Last Telegram callback/admin/navigation guard installed")
    except Exception:
    ...
    try:
        ...
        if not getattr(RB, "_last_messenger_rubika_guard", False):
            ...
            def stable_send(chat, text, r=None):
                ...
                if r:
                    payload.update(chat_keypad_type="New", chat_keypad={"rows": RB.rows(r), "resize_keyboard": True, "one_time_keyboard": False})
                last_error = None
                for attempt in range(3):
                    try:
                        ...
                        if isinstance(data, dict) and data.get("status") == "TOO_REQUESTS":
                            last_error = RuntimeError("TOO_REQUESTS"); time.sleep(6.0 + attempt * 4.0); continue
                        return data
                    except Exception as exc:
                        last_error = exc
                        if attempt < 2: time.sleep(4.0 + attempt * 3.0)
