...
One stable rule:
- only the persistent ReplyKeyboard button below the chat is ``🔄 شروع مجدد``;
- every normal menu/action is an inline button;
- inline callbacks are persisted in SQLite and dispatched by their saved label,
  so they remain valid after a Railway restart.
"""
...
def _init_db(B):
    B.db.conn.execute(
        "CREATE TABLE IF NOT EXISTS ui2_callbacks(token TEXT PRIMARY KEY,user_id TEXT NOT NULL,label TEXT NOT NULL,lang TEXT,status TEXT,created_at TEXT NOT NULL)"
    )
    for col in ("lang", "status"):
        try:
            B.db.conn.execute(f"ALTER TABLE ui2_callbacks ADD COLUMN {col} TEXT")
        except Exception:
            pass
    B.db.conn.commit()
...
def _token(B, uid, label):
    ...
    B.db.conn.execute(
        "INSERT INTO ui2_callbacks(token,user_id,label,lang,status,created_at) VALUES(?,?,?,?,?,?)",
        (token, str(uid or ""), str(label), st.get("lang"), st.get("status"), B.now()),
    )
    B.db.conn.commit()
    return token
...
async def _wallet(update, context, B):
    ...
    await update.effective_message.reply_text(
        "💰 کیف پول من\n\nموجودی کیف پول شما فعلاً صفر است.",
...
async def _complaint_text(update, context, B):
    ...
    if st.get("mode") != "ui2_complaint":
        return
    text = (update.message.text or "").strip()
    if not text:
        return await update.message.reply_text("❌ متن شکایت خالی است.", reply_markup=B.cancel_kb(st.get("lang", "fa")))
    ...
    st["mode"] = None
    return await update.message.reply_text("✅ شکایت شما برای مدیریت ارسال شد.", reply_markup=B.main(uid))
...
async def _dispatch(update, context, B, label):
    ...
    if label == "🛠 پنل مدیریت بات":
        ...
        return await A._callback(
            SimpleNamespace(
                callback_query=SimpleNamespace(data="adm:menu", from_user=q.from_user, message=q.message),
                effective_user=q.from_user,
            ), context, B
        )

    # Partner panel actions are dispatched directly so they do not depend on
    # whichever legacy router happened to be installed last.
    if label == "➕ شارژ حساب":
    ...
    if label == "🏛 حل مشکل سامانه دولت من":
        return await B.gov(fake, context)
    if label == "🔎 پیگیری کد":
        return await B.ptrack(fake, context)
    if label == "📋 سوابق":
        return await B.phistory(fake, context)
    if label == "💰 موجودی":
        if st.get("partner_id"):
            row = B.db.conn.execute("SELECT balance FROM partners WHERE id=?", (st["partner_id"],)).fetchone()
    ...
    if label == "🎫 تیکت به مدیریت":
        # The business feature text handler owns the actual ticket setup.
        from telegram_business_features import _send_ticket_prompt
        return await _send_ticket_prompt(fake, B)

    # Public/service buttons.
    if label == "🪪 فیدای غیر حضوری":
        return await B.fida(fake, context)
    if label == "🖨 خدمات چاپ":
        return await B.prt(fake, context)
    if label == "🪪 حل مشکل ورود اتباع دولت من":
        return await B.gov(fake, context)
    ...
    if label == "📞 تماس با ما":
        return await q.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💬 ارتباط با پشتیبانی", url="https://t.me/Good_ok_2000")]]),
        )
    ...
    # Any feature-specific text route remains available as the final fallback.
...
def install(app, B):
    ...
    async def bind(update, context):
        try:
            _CURRENT_UID.set(getattr(update.effective_user, "id", None))
        except Exception:
    ...
    # Keep the admin menu inline as well. The admin feature itself is installed
    # immediately after this layer, so resolve its menu lazily at call time.
    def admin_menu():
        try:
            import telegram_admin_plus as A
            return A._admin_menu()
        except Exception:
            return inline([["⬅️ منوی اصلی"]], B)
    B.amenu = admin_menu

    # Partner keyboard is owned here; no ReplyKeyboard is allowed for normal
    # options anywhere in the Telegram UI.
    try:
    ...
    async def start(update, context):
        ...
        try:
            await update.message.reply_text("دسترسی سریع:", reply_markup=restart_keyboard())
        except Exception:
            try:
                await update.effective_message.reply_text("دسترسی سریع:", reply_markup=restart_keyboard())
            except Exception:
                pass
        return result
    B.start = start
    ...
    async def callback(update, context):
        ...
        row = B.db.conn.execute(
            "SELECT user_id,label,lang,status FROM ui2_callbacks WHERE token=?", (token,)
        ).fetchone()
        if not row:
            await q.answer("این گزینه منقضی شده است.", show_alert=True)
            return await q.message.reply_text("🔄 لطفاً «شروع مجدد» را بزنید.", reply_markup=restart_keyboard())
