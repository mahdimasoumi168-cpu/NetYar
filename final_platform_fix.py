...
# Telegram: convert every legacy reply keyboard to inline buttons and route clicks safely.
try:
    import bot as B
    ...
    async def _inline_text_callback(update,context):
        ...
        if label is None:
            await q.answer("این گزینه منقضی شده؛ لطفاً منوی جدید را انتخاب کنید.",show_alert=False);return
        await q.answer()
        try:
            await _remove_old_telegram_keyboard(q.message)
            original_text=getattr(q.message,"text",None);object.__setattr__(q.message,"text",label)
...
# Rubika: inline keypad only, controlled sending, correct webhook URL normalization,
# and a polling fallback if Rubika rejects Railway's webhook URL.
try:
    ...
    def _rubika_send(chat,text,r=None):
        ...
        for attempt in range(4):
            try:
                ...
                z=RB.HTTP.post(f"{RB.BASE}/sendMessage",json=p,timeout=20);z.raise_for_status();data=z.json()
                if isinstance(data,dict) and data.get("status")=="TOO_REQUESTS":
                    last_exc=RuntimeError("TOO_REQUESTS");time.sleep(min(30.0,8.0*(attempt+1)));continue
                return data
    ...
    async def _rubika_poll(server):
        offset_id=None
        log.warning("Rubika webhook unavailable; immediate getUpdates fallback started")
        while True:
            try:
                payload={"limit":20}
                ...
                if next_offset:offset_id=str(next_offset)
                for update in updates:
                    if isinstance(update,dict):await server._run_rubika(update,RB)
                if not updates:await asyncio.sleep(0.5)
            except asyncio.CancelledError:return
            except Exception:
                log.exception("Rubika polling fallback failed");await asyncio.sleep(3)
    ...
    try:
        ...
        async def _ensure_rubika_fallback():
            ...
            if task is None or task.done():
                _server._rubika_final_polling_task=asyncio.create_task(_rubika_poll(_server))
                _server.rubika_ready=True
                log.warning("Rubika marked online through getUpdates fallback")
        _original_startup=getattr(_server,"startup",None)
        # Install one background safety task without replacing the existing startup flow.
        if not getattr(_server,"_final_rubika_fallback_installed",False):
            _server._final_rubika_fallback_installed=True
            _server._final_rubika_fallback_task=asyncio.create_task(_ensure_rubika_fallback()) if hasattr(asyncio,"get_running_loop") else None
