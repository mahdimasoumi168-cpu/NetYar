"""Canonical Telegram access gate: Tehran business hours and persistent night access."""
...
def is_night_worker(B,uid):
    ...
    return _setting(B,PREFIX+str(row["id"]),"0")=="1"
...
def enforce_24x7(B):
    """Bind legacy gates to this single source of truth without resetting persisted settings."""
    ...
    def _open(*args,**kwargs):
        ...
        return night_access_open(B,uid) if uid is not None else _clock_is_open(B)
    ...
    for name in (
        "telegram_business_hours_guard","telegram_night_shift_v2","telegram_night_shift",
        "telegram_final_control","telegram_persian_offhours_lock","telegram_absolute_offhours_guard",
...
def install(app,B):
    if getattr(B,"_offhours_partner_gate_v12",False):
        return True
    B._netyar_24x7=False
    B._netyar_night_gate_authoritative=True
    B._offhours_partner_gate_v12=True
    return True
