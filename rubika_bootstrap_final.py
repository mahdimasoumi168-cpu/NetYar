...
This wrapper exists so Rubika can start reliably even when an older server.py
still defaults to Telegram-style/polling behavior. It never changes Telegram
handlers or polling.
"""
import logging
...
def install(server_module):
    ...
    async def initialize_with_rubika():
        ...
        try:
            ...
            # Install only Rubika-side compatibility layers. Each layer is
            # optional and isolated so one legacy patch cannot stop startup.
            for name in (
                "rubika_fix",
                "rubika_core_compat",
                "rubika_runtime_fix",
            ...
            # Railway is an HTTP service: use the webhook endpoint explicitly.
