...
def install(app,B):
 ...
 def partner_for(r):
  ...
  try:
   u=B.db.conn.execute("SELECT external_id FROM users WHERE id=? AND platform='telegram'",(r["user_id"],)).fetchone()
  ...
  except Exception: log.exception("exact request-owner partner resolution failed")
  try:
 ...
 async def ask(update,context):
  ...
  try:
   ...
   # Prefer the exact Telegram chat captured when the request was created.
   chat=exact_request_chat(rid)
   p=partner_for(r)
   if not chat and p: chat=partner_chat(p)
   if not p and not chat:await q.answer("کاربری که این درخواست را ثبت کرده پیدا نشد",show_alert=True);raise ApplicationHandlerStop
   if not chat:await q.answer("چت کاربر ثبت نشده؛ کاربر باید یک‌بار وارد ربات شود.",show_alert=True);raise ApplicationHandlerStop
   ...
   n+=1;B.db.set_setting(f"partner_code_attempts_{rid}",str(n));B.db.set_setting(f"partner_code_request_admin_{rid}",str(q.from_user.id))
   if p:
    B.db.set_setting(f"request_partner_{rid}",str(p["id"]));B.db.set_setting(f"partner_code_request_{p['id']}",f"{rid}|{r['tracking_code']}")
   B.db.conn.execute("UPDATE requests SET status='awaiting_partner_code',updated_at=? WHERE id=?",(B.now(),rid));B.db.conn.commit()
   st=state(chat);st.update(partner_id=int(p["id"]) if p else st.get("partner_id"),partner_active=True,mode="partner_send_code",code_request_rid=rid,code_request_admin=int(q.from_user.id),lang=st.get("lang","fa"))
   msg=("👔 مدیریت\n\n📨 درخواست کد خدمت\n"f"🎫 کد پیگیری: {r['tracking_code']}\n🧾 خدمت: {r['service_key']}\n\nلطفاً کد خدمت/کد انجام کار را همین‌جا ارسال کنید. فقط متن کد را بفرستید.")
   err=None
   for i in range(3):
    try:
     await context.bot.send_message(chat_id=chat,text=msg,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ لغو",callback_data=f"pc:x:{rid}:{p['id'] if p else 0}")]]));err=None;break
    except Exception as e:err=e;await asyncio.sleep(.7*(i+1))
   if err:raise err
   await q.answer("✅ درخواست کد برای کاربر ثبت‌کننده ارسال شد")
   await q.message.reply_text(f"✅ درخواست کد برای «{p['name'] or p['phone'] if p else chat}» ارسال شد.\n🎫 {r['tracking_code']}")
  except ApplicationHandlerStop:raise
  except Exception:
   log.exception("ask partner code failed");await q.answer("❌ ارسال درخواست کد ناموفق بود.",show_alert=True)
 ...
 async def reply(update,context):
  ...
  if st.get("partner_id") and str(st.get("partner_id"))!=str(r["user_id"]):return
  B.db.answer(rid,"partner_code",answer=text);B.db.conn.commit()
  ...
  p=B.db.conn.execute("SELECT name,phone FROM partners WHERE id=?",(int(st["partner_id"]),)).fetchone() if st.get("partner_id") else None
  msg=("🔐 کد از کاربر درخواست‌شده دریافت شد\n\n"f"👤 کاربر ثبت‌کننده: {p['name'] if p else '-'}\n📱 شماره: {p['phone'] if p else '-'}\n🎫 کد پیگیری: {r['tracking_code']}\n🧾 خدمت: {r['service_key']}\n🔑 کد: {text}")
  sent=False
  for x in recipients:
   if not x:continue
   for i in range(3):
  ...
  if st.get("partner_id"):B.db.set_setting(f"partner_code_request_{st['partner_id']}","")
