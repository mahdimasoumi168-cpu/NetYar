...
def normalize(u):
    ...
    if not text and isinstance(aux,dict): text=str(aux.get('button_id') or aux.get('button_id_string') or '').strip()
    fid=''; f=msg.get('file')
...
def admin_flow(uid,chat,text):
    ...
    if text=='a_requests':
        ...
        lines=['📋 آخرین درخواست‌ها:','']
        for r in rows: lines.append(f"#{r['id']} | {r['tracking_code']} | {r['service_key']} | {r['status']} | {int(r['amount']):,} تومان | {r['payment_status']}")
        rows_btn=[]
        for r in rows[:10]: rows_btn.append([(f'req:{r["id"]}:processing',f'▶️ شروع #{r["id"]}'),(f'req:{r["id"]}:done',f'✅ اتمام #{r["id"]}')])
        rows_btn.append([('req:back','↩️ مدیریت')]); send(chat,'\n'.join(lines),key(rows_btn)); return True
    if text.startswith('req:'):
        parts=text.split(':')
        if len(parts)==3 and parts[1].isdigit():
            rid=int(parts[1]); status=parts[2]
            if status=='back': send(chat,'🛠 مدیریت:',admin_menu()); return True
            labels={'processing':'در حال انجام','done':'انجام شد'}; label=labels.get(status,status)
            ...
            db.conn.execute('UPDATE requests SET status=?,updated_at=? WHERE id=?',(label,db.conn.execute('SELECT datetime(\'now\')').fetchone()[0],rid)); db.conn.commit()
            partner_id=None
            ...
            if partner_id: notify_partner(partner_id,f"🔔 وضعیت درخواست شما تغییر کرد.\n🎫 کد پیگیری: {row['tracking_code']}\n📌 وضعیت جدید: {label}")
...
def main():
    ...
    offset=None; log.info('Rubika polling started: stable v2')
    while True:
        try:
            payload={'limit':50}
            ...
            for u in updates:
                ...
                try:
                    ...
                    if text.lower().startswith('/start'):
                        S[uid]={'lang':'fa','step':'language'}; send(chat,'🌐 زبان را انتخاب کنید:',key([[('1','🇮🇷 فارسی'),('2','🇬🇧 English'),('3','🇸🇦 العربية')]])); continue
                    ...
                    # Make language selection deterministic even when Rubika sends button_id instead of button text.
                    if S.get(uid,{}).get('step')=='language':
                        lv=language_value(text)
                        if lv: text=lv
                    before=S.get(uid,{}).copy()
                    handle(uid,chat,text or fid)
                    after=S.get(uid,{})
                    # Save the partner's current chat id for automatic status notifications.
                    if before.get('step')=='partner_pass' and after.get('partner_id'):
