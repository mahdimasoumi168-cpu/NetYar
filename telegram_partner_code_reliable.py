...
Admin -> assigned partner -> partner sends code -> exact requesting admin receives
it. The flow is isolated by request id and runs early so generic message handlers
cannot consume the partner's code first.
"""
...
def install(app,B):
 ...
 def state(uid):
  return B.S.setdefault(uid,{}) if uid in B.S else B.S.setdefault(str(uid),{})
 ...
 async def ask(update,context):
  ...
  try:
   ...
   n+=1
   for k,v in ((f"partner_code_attempts_{rid}",str(n)),(f"partner_code_request_admin_{rid}",str(q.from_user.id)),(f"request_partner_{rid}",str(p['id'])),(f"partner_code_request_{p['id']",f"{rid}|{r['tracking_code']}")):
 ...
 async def reply(update,context):
  ...
  if not sent:log.exception("code could not be delivered to admin: %s",rid)
