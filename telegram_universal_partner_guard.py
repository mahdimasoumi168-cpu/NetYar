    ...
    async def text_owner(update, context):
        ...
        if mode in {"p_phone", "partner_phone"} or st.get("step") == "partner_phone":
            ...
            if not re.fullmatch(r"09\d{9}", phone):
                ...
                await msg.reply_text("❌ شماره موبایل صحیح نیست.\n\n📱 شماره موبایل اختصاصی همکار را دوباره وارد کنید:", reply_markup=B.cancel_kb(st.get("lang", "fa")))
                raise ApplicationHandlerStop
            p = _partner(B, phone)
            if not p:
                st.update(mode="p_phone", step="partner_phone")
                await msg.reply_text("❌ این شماره به همکار فعال اختصاص ندارد.\n\n📱 شماره را دوباره وارد کنید:", reply_markup=B.cancel_kb(st.get("lang", "fa")))
                raise ApplicationHandlerStop
            st.update(phone=phone, partner_phone=phone, partner_id=int(p["id"]), partner_active=True, mode="p_pass", step="partner_pass", pending_partner_id=int(p["id"]))
