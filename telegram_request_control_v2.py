"""Canonical manager request controls.

One button = one action. State-changing actions return to the full manager
panel instead of re-opening the same request card. Partner-code requests use
the single reliable ``panel:askcode:<id>`` flow.
"""
from telegram import InlineKeyboardMarkup, InlineKeyboardButton
from telegram.ext import CallbackQueryHandler, MessageHandler, filters, ApplicationHandlerStop


def partner_uid(B,pid):
    r=B.db.conn.execute("SELECT telegram_user_id FROM partner_telegram_links WHERE partner_id=?",(pid,)).fetchone()
    return int(r["telegram_user_id"]) if r and str(r["telegram_user_id"]).isdigit() else None


def menu(rid, paid=False):
    rows=[[InlineKeyboardButton("🔎 جزئیات کامل",callback_data=f"rq:detail:{rid}")],
          [InlineKeyboardButton("📨 درخواست کد از همکار",callback_data=f"panel:askcode:{rid}")]]
    if paid:
        rows.append([InlineKeyboardButton("⏳ بررسی اولیه",callback_data=f"rq:review:{rid}"),InlineKeyboardButton("✅ انجام شد",callback_data=f"rq:approve:{rid}")])
    else:
        rows.append([InlineKeyboardButton("💰 تأیید دریافت وجه",callback_data=f"rq:payconfirm:{rid}")])
        rows.append([InlineKeyboardButton("⏳ بررسی اولیه",callback_data=f"rq:review:{rid}")])
    rows.append([InlineKeyboardButton("❌ رد درخواست",callback_data=f"rq:reject:{rid}")])
    return InlineKeyboardMarkup(rows)


def admin_panel(B):
    try: return B.amenu()
    except Exception: return None


async def cb(update,context,B):
    q=update.callback_query
    if not q:return
    d=(q.data or "").split(":")
    if len(d)<3 or d[0]!="rq":return
    if not B.admin(q.from_user.id):
        await q.answer("دسترسی ندارید",show_alert=True); raise ApplicationHandlerStop
    await q.answer()
    try: rid=int(d[2])
    except Exception:
        await q.message.reply_text("❌ شناسه درخواست نامعتبر است.",reply_markup=admin_panel(B)); raise ApplicationHandlerStop
    r=B.db.conn.execute("SELECT * FROM requests WHERE id=?",(rid,)).fetchone()
    if not r:
        await q.message.reply_text("❌ درخواست پیدا نشد.",reply_markup=admin_panel(B)); raise ApplicationHandlerStop
    action=d[1]; paid=str(r["payment_status"] or "").lower()=="paid"

    if action=="detail":
        ans=B.db.conn.execute("SELECT key,answer FROM request_answers WHERE request_id=?",(rid,)).fetchall()
        lines=[f"🎫 کد پیگیری: {r['tracking_code']}",f"🧾 خدمت: {r['service_key']}",f"📌 وضعیت: {r['status']}",f"💰 مبلغ: {int(r['amount'] or 0):,} تومان",f"💳 پرداخت: {'تأیید شده' if paid else 'تأیید نشده'}"]
        lines += [f"{a['key']}: {a['answer']}" for a in ans if a['answer']]
        await q.message.reply_text("🔎 <b>جزئیات کامل درخواست</b>\n\n"+"\n".join(lines),reply_markup=menu(rid,paid),parse_mode="HTML")
        raise ApplicationHandlerStop

    if action=="payconfirm":
        if not paid:
            B.db.conn.execute("UPDATE requests SET payment_status='paid',payment_method='manual_admin',status='reviewing',updated_at=? WHERE id=?",(B.now(),rid));B.db.conn.commit()
            text="✅ دریافت وجه تأیید شد و درخواست وارد بررسی اولیه شد."
        else: text="ℹ️ این پرداخت قبلاً تأیید شده بود."
        await q.message.reply_text(text,reply_markup=admin_panel(B)); raise ApplicationHandlerStop

    if action in {"review","approve","reject"}:
        if action=="approve" and not paid:
            await q.message.reply_text("⛔ ابتدا دریافت وجه را تأیید کنید.",reply_markup=admin_panel(B)); raise ApplicationHandlerStop
        status={"review":"reviewing","approve":"completed","reject":"rejected"}[action]
        B.db.conn.execute("UPDATE requests SET status=?,updated_at=? WHERE id=?",(status,B.now(),rid));B.db.conn.commit()
        labels={"review":"⏳ درخواست برای بررسی اولیه ثبت شد.","approve":"✅ درخواست انجام‌شده ثبت شد.","reject":"❌ درخواست رد شد."}
        await q.message.reply_text(labels[action],reply_markup=admin_panel(B)); raise ApplicationHandlerStop

    # Old rq:ask callbacks are retained only for compatibility; the visible
    # button now uses the single reliable panel:askcode handler.
    if action=="ask":
        await q.message.reply_text("📨 برای درخواست کد، همان گزینه «درخواست کد از همکار» را یک‌بار بزنید.",reply_markup=admin_panel(B)); raise ApplicationHandlerStop

    raise ApplicationHandlerStop


async def code_cb(update,context,B):
    q=update.callback_query
    if not q:return
    d=(q.data or "").split(":")
    if len(d)<4 or d[0]!="rqcode":return
    rid=int(d[2]);stage=int(d[3]);uid=q.from_user.id;st=B.S.setdefault(uid,{})
    if d[1]=="send":
        r=B.db.conn.execute("SELECT user_id FROM requests WHERE id=?",(rid,)).fetchone()
        if not r or str(st.get("partner_id"))!=str(r["user_id"]): await q.answer("این درخواست متعلق به شما نیست.",show_alert=True); raise ApplicationHandlerStop
        st.update(mode="partner_send_code",code_request_id=rid,code_stage=stage); await q.answer(); await q.message.reply_text(f"🔐 کد مرحله {stage} را همینجا ارسال کنید:"); raise ApplicationHandlerStop
    if d[1]=="next" and B.admin(uid):
        pid=st.get("code_partner_id");target=partner_uid(B,pid)
        if not target: await q.message.reply_text("❌ همکار در دسترس نیست.",reply_markup=admin_panel(B)); raise ApplicationHandlerStop
        nxt=min(10,stage+1);st["code_stage"]=nxt
        await context.bot.send_message(target,f"🔐 مرحله {nxt} از ۱۰\nکد مرحله {nxt} را ارسال کنید.",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"📨 ارسال کد مرحله {nxt}",callback_data=f"rqcode:send:{rid}:{nxt}")]]))
        await q.message.reply_text(f"📨 مرحله {nxt} برای همکار ارسال شد."); raise ApplicationHandlerStop
    if d[1]=="finish" and B.admin(uid):
        await q.message.reply_text("✅ دریافت کدها پایان یافت. پروژه این درخواست از این مرحله خارج شد.",reply_markup=admin_panel(B)); raise ApplicationHandlerStop
    raise ApplicationHandlerStop


async def code_text(update,context,B):
    if not update.message:return
    uid=update.effective_user.id;st=B.S.setdefault(uid,{})
    if st.get("mode")!="partner_send_code":return
    rid=int(st.get("code_request_id"));stage=int(st.get("code_stage",1));text=(update.message.text or "").strip()
    r=B.db.conn.execute("SELECT user_id,tracking_code FROM requests WHERE id=?",(rid,)).fetchone()
    if not r or str(st.get("partner_id"))!=str(r["user_id"]):return
    if not text:
        await update.message.reply_text("❌ کد خالی است؛ کد خدمت را ارسال کنید."); raise ApplicationHandlerStop
    B.db.conn.execute("CREATE TABLE IF NOT EXISTS request_partner_codes(request_id INTEGER,stage INTEGER,code TEXT,created_at TEXT,PRIMARY KEY(request_id,stage))")
    B.db.conn.execute("INSERT OR REPLACE INTO request_partner_codes VALUES(?,?,?,?)",(rid,stage,text,B.now()));B.db.conn.commit()
    for aid in B.ADM:
        try:
            buttons=[]
            if stage<10: buttons.append(InlineKeyboardButton(f"➡️ مرحله {stage+1}",callback_data=f"rqcode:next:{rid}:{stage}"))
            buttons.append(InlineKeyboardButton("✅ پایان",callback_data=f"rqcode:finish:{rid}:{stage}"))
            await context.bot.send_message(int(aid),f"🔐 کد همکار دریافت شد\n🎫 {r['tracking_code']}\nمرحله {stage} از ۱۰\nکد: {text}",reply_markup=InlineKeyboardMarkup([buttons]))
        except Exception: pass
    st["mode"]=None
    await update.message.reply_text("✅ کد خدمت دریافت شد و برای مدیریت ارسال شد.")
    raise ApplicationHandlerStop


def install(app,B):
    if getattr(B,"_request_control_v2",False):return
    B.db.conn.execute("CREATE TABLE IF NOT EXISTS request_partner_codes(request_id INTEGER,stage INTEGER,code TEXT,created_at TEXT,PRIMARY KEY(request_id,stage))");B.db.conn.commit()
    app.add_handler(CallbackQueryHandler(lambda u,c:cb(u,c,B),pattern=r"^rq:"),group=-50)
    app.add_handler(CallbackQueryHandler(lambda u,c:code_cb(u,c,B),pattern=r"^rqcode:"),group=-49)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND,lambda u,c:code_text(u,c,B)),group=-48)
    B._request_control_v2=True
