...
async def _partner_entry(update,context,B):
    ...
    if text in {"👥 پنل همکاران","🔵 👥 پنل همکاران","پنل همکاران"}:
        ...
        if p and st.get("partner_id")==p["id"]:
            return await msg.reply_text(f"👥 پنل همکاران\n👤 {p['name']}\n📱 {p['phone']}\n💰 اعتبار: {int(p['balance'] or 0):,} تومان",reply_markup=B.partner_kb())
        pending=_pending(B,uid)
        if pending:
            return await msg.reply_text(f"⏳ درخواست همکاری شما در انتظار بررسی مدیریت است.\n\n🎫 کد پیگیری: {pending['tracking_code']}",reply_markup=_app_menu(B))
        st.update(partner_gate_mode="phone",partner_registration={})
        return await msg.reply_text("👥 ورود به پنل همکاران\n\n📱 شماره موبایل خود را وارد کنید:",reply_markup=B.cancel_kb(st.get("lang","fa")))
    ...
    if mode=="phone":
        ...
        if row:
            ...
            return await msg.reply_text("🔐 رمز عبور همکار را وارد کنید:",reply_markup=B.cancel_kb(st.get("lang","fa")))
        ...
        if pending:
            ...
            return await msg.reply_text(f"⏳ برای این شماره یک درخواست همکاری در انتظار بررسی است.\n🎫 کد پیگیری: {pending['tracking_code']}",reply_markup=_app_menu(B))
        reg["phone"]=p;st["partner_gate_mode"]="password"
        return await msg.reply_text("🔐 یک رمز عبور جدید برای پنل همکاران انتخاب کنید:\n\nحداقل ۶ کاراکتر.",reply_markup=B.cancel_kb(st.get("lang","fa")))
    if mode=="password":
        if len(text)<6:return await msg.reply_text("❌ رمز عبور باید حداقل ۶ کاراکتر باشد.",reply_markup=B.cancel_kb(st.get("lang","fa")))
        reg["password"]=text;st["partner_gate_mode"]="office"
        return await msg.reply_text("🏢 نام دفتر یا محل فعالیت را وارد کنید:",reply_markup=B.cancel_kb(st.get("lang","fa")))
    if mode=="office":
        if len(text)<2:return await msg.reply_text("❌ نام دفتر را کامل وارد کنید.",reply_markup=B.cancel_kb(st.get("lang","fa")))
        reg["office_name"]=text
        from core import hash_password
        code=_tracking(B)
        cur=B.db.conn.execute("INSERT INTO partner_applications(telegram_user_id,phone,office_name,password_hash,status,tracking_code,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)",(str(uid),reg["phone"],reg["office_name"],hash_password(reg["password"]),"pending",code,B.now(),B.now()))
        ...
        return await msg.reply_text(f"✅ درخواست همکاری شما ثبت شد.\n\n🎫 کد پیگیری: {code}\n⏳ وضعیت: در انتظار بررسی مدیریت\n\nتا زمان تأیید، هیچ‌یک از خدمات پنل همکاران برای شما فعال نیست.",reply_markup=_app_menu(B))
...
async def _text(update,context,B):
    ...
    if text=="🤝 درخواست همکاری":
        p=_pending(B,uid)
        if p:return await update.message.reply_text(f"⏳ درخواست شما قبلاً ثبت شده است.\n🎫 {p['tracking_code']}",reply_markup=_app_menu(B))
        st["partner_gate_mode"]="phone";st["partner_registration"]={}
        return await update.message.reply_text("📱 شماره موبایل خود را وارد کنید:",reply_markup=B.cancel_kb(st.get("lang","fa")))
    if text=="🔎 پیگیری درخواست همکاری":
        ...
        if not p:return await update.message.reply_text("ℹ️ هنوز درخواست همکاری ثبت نکرده‌اید.",reply_markup=_app_menu(B))
        status={"pending":"⏳ در انتظار بررسی","approved":"✅ تأیید شده","rejected":"❌ رد شده"}.get(p["status"],p["status"])
        return await update.message.reply_text(f"🎫 کد پیگیری: {p['tracking_code']}\n🏢 دفتر: {p['office_name']}\n📌 وضعیت: {status}",reply_markup=_app_menu(B))
    if B.admin(uid) and text=="🤝 درخواست‌های همکاری":
        return await update.message.reply_text("🤝 درخواست‌های همکاری در انتظار بررسی:",reply_markup=_admin_apps(B))
...
async def _cb(update,context,B):
    ...
    if d[1]=="none":return
    if not B.admin(uid):return await q.message.reply_text("❌ دسترسی ندارید.")
    ...
    if d[1]=="view":
        ...
        return await q.message.reply_text(f"🤝 درخواست همکاری\n\n🎫 {r['tracking_code']}\n🏢 نام دفتر: {r['office_name']}\n📱 شماره: {r['phone']}\n🆔 تلگرام: {r['telegram_user_id']}\n📅 ثبت: {r['created_at']}",reply_markup=kb)
    if d[1] in {"approve","reject"}:
        if r["status"]!="pending":return await q.message.reply_text("ℹ️ این درخواست قبلاً بررسی شده است.")
        if d[1]=="reject":
            ...
            return await q.message.reply_text("❌ درخواست رد شد.")
        from core import hash_password
        ...
        return await q.message.reply_text(f"✅ درخواست {r['tracking_code']} تأیید شد و پنل همکار فعال شد.")
...
def install(app,B):
    ...
    def restricted_partner_kb(lang="fa"):
        return B.kb([["🤝 درخواست همکاری"],["🔎 پیگیری درخواست همکاری"] ,[CANCEL]])
