"""Strict night-worker access and admin management."""
...
def worker(B,pid):
    return bool(pid and B.db.setting(PREFIX+str(pid),"0")=="1")
...
def assigned(B,uid):
    ...
    return worker_phone(B,st.get("phone"))
...
def allowed(B,uid,update=None):
    ...
    if update:
        ...
        txt=(getattr(msg,"text","") if msg else "") or ""
        # Allow an assigned worker to start authentication by entering their
        # registered partner phone; do NOT allow arbitrary users into partner login.
        if worker_phone(B,txt.strip()): return True
        if txt.strip() in {RESTART,"شروع مجدد","/start","start"}: return True
    return False


def closed():
    if is_friday():
        return "⛔ امروز جمعه است و ربات خدمات عادی تعطیل می‌باشد.\n\nبرای استفاده از خدمات لطفاً در ساعت کاری مراجعه کنید."
...
def install(app,B):
    ...
    async def partner(update,context):
        ...
        if not open_now() and not allowed(B,uid,update):
            return await update.effective_message.reply_text(closed(),reply_markup=markup(B,uid))
    ...
    async def cb(update,context):
        ...
        if d[1]=="menu":
            rows=B.db.conn.execute("SELECT p.id,p.name,p.phone,COALESCE(n.enabled,0) enabled FROM partners p LEFT JOIN night_workers n ON n.partner_id=p.id WHERE p.active=1 ORDER BY p.id DESC").fetchall()
            buttons=[]
            for r in rows:
                buttons.append([InlineKeyboardButton(("🟢 " if r["enabled"] else "⚪ ")+f"{r['name'] or r['phone']}",callback_data=f"night2:toggle:{r['id']}")])
            ...
            return await q.message.reply_text("🌙 مدیریت همکاران شب‌کار\n\n🟢 فعال = اجازه ورود و فعالیت خارج از ساعت کاری و جمعه\n⚪ غیرفعال = فقط ساعت کاری عادی",reply_markup=InlineKeyboardMarkup(buttons))
        if d[1]=="add":
            st["night_mode"]="add"
            return await q.message.reply_text("📱 شماره موبایل همکار شب‌کار را وارد کنید:")
        if d[1]=="toggle":
            pid=int(d[2]); en=not worker(B,pid)
            B.db.set_setting(PREFIX+str(pid),"1" if en else "0")
            B.db.conn.execute("INSERT OR REPLACE INTO night_workers VALUES(?,?,?)",(pid,1 if en else 0,B.now()))
            B.db.conn.commit()
            return await q.message.reply_text("✅ دسترسی شب‌کاری فعال شد." if en else "✅ دسترسی شب‌کاری لغو شد.",reply_markup=menu())
    ...
    async def text(update,context):
        ...
        p=phone(update.message.text)
        row=B.db.conn.execute("SELECT id,name FROM partners WHERE phone=? AND active=1",(p,)).fetchone() if p else None
        if not row: return await update.message.reply_text("❌ همکار فعال با این شماره پیدا نشد.")
        B.db.set_setting(PREFIX+str(row["id"]),"1")
        B.db.conn.execute("INSERT OR REPLACE INTO night_workers VALUES(?,?,?)",(row["id"],1,B.now())); B.db.conn.commit()
        st["night_mode"]=None
    ...
    async def gate(update,context):
        ...
        if txt in {"/start","/restart","start",RESTART}: return
        await msg.reply_text(closed(),reply_markup=markup(B,update.effective_user.id))
