...
One canonical admin panel: no duplicate management buttons. Every adm:*
callback is consumed by the canonical admin feature owner so legacy callback
handlers cannot execute a second action or replace the visible panel.
"""
import re
...
async def _callback(update, context, B):
    ...
    # All other adm:* callbacks have one feature owner: telegram_admin_plus.
    try:
...
async def _text(update, context, B):
    ...
    if st.get("admin_plus_mode")!="add_partner": return
    ...
    if not re.fullmatch(r"09\d{9}",phone):
        await msg.reply_text("❌ شماره موبایل معتبر نیست.",reply_markup=menu()); raise ApplicationHandlerStop
    try:
        B.db.add_partner(phone,parts[1],parts[2]); st["admin_plus_mode"]=None
