"""Canonical Rubika Iranian subscriber menu and complaint forwarding."""
import os
import logging
...
def install(rb):
    if getattr(rb,"_iranian_complaints_installed",False):
        return
    ...
    def handle(uid,chat,x,u):
        ...
        if st.get("status")=="iranian" and step in {"iranian_menu","menu"}:
            ...
            if x in {"4","📞 تماس با ما","📞 Contact us","📞 اتصل بنا"}:
                return rb.send(chat,"📞 تماس با ما\n\nآیدی پشتیبانی: @"+CONTACT_USERNAME,[[('0','⬅️ بازگشت')]])
            if x in {"5","📝 ثبت شکایت","📝 Complaint","📝 شكوى"}:
                st["step"]="iranian_complaint";return rb.send(chat,"📝 متن شکایت یا انتقاد خود را ارسال کنید:",[[('0',rb.CANCEL)]])
        ...
        if st.get("status")=="iranian" and step=="iranian_complaint":
            ...
            st["step"]="iranian_menu";return rb.send(chat,"✅ شکایت شما برای مدیریت ارسال شد.",iranian_rows(uid))
    ...
    rb.iranian_rows=iranian_rows
    rb._iranian_handle=handle
    rb._iranian_main_rows=main_rows
    rb._iranian_complaints_installed=True

def restore(rb):
    """Re-apply the canonical Iranian routing after generic Rubika patches."""
    if getattr(rb,"_iranian_complaints_installed",False):
        if getattr(rb,"_iranian_handle",None): rb.handle=rb._iranian_handle
        if getattr(rb,"_iranian_main_rows",None): rb.main_rows=rb._iranian_main_rows
        return True
    install(rb)
    return True
