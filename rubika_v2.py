...
def admin(uid,chat,x):
    ...
    if step=="bot_api":
        p=st.get("bot_platform"); name=st.get("bot_name") or p; db.set_setting(f"bot_{p}_name",name); db.set_setting(f"bot_{p}_api",x); st["step"]="admin"; send(chat,T(uid,"bot_saved",name=name,platform=p),admin_rows()); return
    if step=="admin":
        ...
        if x=="2": send(chat,"💰 شارژها",admin_rows()); return
        if x=="3": send(chat,"📋 درخواست‌ها",admin_rows()); return
        if x=="4": send(chat,"💳 پرداخت‌ها",admin_rows()); return
        ...
        if x=="7": send(chat,"🤖 بات‌های متصل",admin_rows()); return
        if x=="8": send(chat,"📊 گزارش",admin_rows()); return
...
def handle(uid,chat,x,u):
    ...
    if step=="citizenship":
        if x in {"1","🪪 اتباع هستم","🪪 Foreign national","🪪 أجنبی"}: st["step"]="main"; send(chat,T(uid,"menu"),main_rows(uid)); return
    ...
    if step=="partner_pass":
        ...
        if not p or not check_password(p["password_hash"],x): st["step"]="partner_phone"; send(chat,T(uid,"bad_login")); return
    ...
    if step=="gov_doc":
        ...
        st["gov_doc"]=docs[x]; st["step"]="gov_files"; st["files"]=[]; send(chat,"📸 تصاویر مدرک را ارسال کنید. برای پاسپورت: صفحه اول و صفحه تمدید را بفرستید.",[[('1',OK),('0',CANCEL)]]); return
    if step=="gov_files":
        if x in {OK,"1"}:
            files=st.get("files",[])
