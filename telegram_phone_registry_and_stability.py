...
PHONE_KEYS = {"phone", "gov_phone", "mobile", "subscriber_phone", "customer_phone", "irancell_phone", "mobile_number", "customer_mobile", "common_phone"}
...
def ensure(B):
    B.db.conn.execute("""CREATE TABLE IF NOT EXISTS subscriber_phones(
        phone TEXT PRIMARY KEY, first_seen TEXT DEFAULT '', last_seen TEXT DEFAULT '',
        use_count INTEGER DEFAULT 1, last_service TEXT DEFAULT '', last_request_id INTEGER DEFAULT 0
    )""")
    B.db.conn.commit()
...
def remember(B, phone, service="", request_id=0):
    ...
    if row:
        B.db.conn.execute("UPDATE subscriber_phones SET last_seen=?,use_count=use_count+1,last_service=?,last_request_id=? WHERE phone=?", (now, service or "", int(request_id or 0), p))
    else:
        B.db.conn.execute("INSERT INTO subscriber_phones(phone,first_seen,last_seen,use_count,last_service,last_request_id) VALUES(?,?,?,?,?,?)", (p, now, now, 1, service or "", int(request_id or 0)))
    B.db.conn.commit()
    return True
...
def admin_menu_markup():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📱 شماره موبایل مشترک", callback_data="adm:phones")],
        ...
        [InlineKeyboardButton("⚙️ تنظیمات", callback_data="adm:settings"), InlineKeyboardButton("⬅️ منوی اصلی", callback_data="adm:main")],
    ])

def install(app, B):
    if getattr(B, "_phone_registry_stability", False): return
    ensure(B)
    try:
        rows = B.db.conn.execute("""SELECT ra.answer,ra.request_id,r.service_key FROM request_answers ra
        ...
        for r in rows: remember(B, r["answer"], r["service_key"], r["request_id"])
    except Exception: pass

    async def admin_menu_override(update, context):
        q = update.callback_query
        if not q or not B.admin(q.from_user.id): return
        await q.answer()
        await q.message.reply_text("🛠 پنل مدیریت کامل\n\nاز منوی زیر بخش موردنظر را انتخاب کنید:", reply_markup=admin_menu_markup())
    ...
    async def admin_phones(update, context):
        ...
        if not q or not B.admin(q.from_user.id): return
        await q.answer()
        rows = B.db.conn.execute("SELECT phone,use_count,last_service,last_seen,last_request_id FROM subscriber_phones ORDER BY last_seen DESC LIMIT 100").fetchall()
        if not rows:
            text = "📱 شماره موبایل مشترک\n\nهنوز شماره‌ای ثبت نشده است."
        else:
            lines = [f"📱 شماره موبایل مشترک\n\nتعداد شماره‌های یکتا: {len(rows)}\n"]
            for i, r in enumerate(rows, 1):
                lines.append(f"{i}. 📱 {r['phone']} | دفعات: {int(r['use_count'] or 0)} | خدمت: {r['last_service'] or '-'}")
            text = "\n".join(lines)
        await q.message.reply_text(text[:3900], reply_markup=back_markup())
        raise ApplicationHandlerStop

    app.add_handler(CallbackQueryHandler(admin_menu_override, pattern=r"^adm:menu$"), group=-30000001)
    ...
    async def phone_guard(update, context):
        ...
        st = B.S.setdefault(user.id, {}); mode = str(st.get("mode") or "")
        if mode not in PHONE_MODES: return
        p = normalize_phone(msg.text)
        if not p: return
        if mode.startswith("gov"):
            st["gov_phone"] = p; st["mode"] = "govv2_dob"; remember(B, p, "government", 0)
            await msg.reply_text("🎂 تاریخ تولد مشترک را وارد کنید:", reply_markup=B.cancel_kb(st.get("lang", "fa")))
            raise ApplicationHandlerStop
        if mode == "irancell_partner_phone":
            st.setdefault("irancell", {})["phone"] = p; remember(B, p, "irancell_sim_issue", 0); st["mode"] = "irancell_partner_document"
