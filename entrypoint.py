...
NETYAR_TELEGRAM_BUILD="2026-09-15-persian-only-offhours-v14"
...
def _install_module(name,app,bot,logger,*,pre=False):
    try:
        ...
        sig=inspect.signature(fn);pos=[p for p in sig.parameters.values() if p.kind in (inspect.Parameter.POSITIONAL_ONLY,inspect.Parameter.POSITIONAL_OR_KEYWORD)];req=[p for p in pos if p.default is inspect.Parameter.empty]
        if len(req)>=2 or len(pos)>=2:fn(app,bot)
        elif len(req)==1 or len(pos)==1:fn(bot)
        else:fn()
...
def _install_telegram_layers(app,bot,logger):
    for n in PRE_TELEGRAM_MODULES:_install_module(n,app,bot,logger,pre=True)
    for n in TELEGRAM_MODULES:_install_module(n,app,bot,logger)
    _install_module("telegram_government_strict_validation",app,bot,logger)
    _install_module("telegram_government_flow_hardening_v4",app,bot,logger)
    _install_module("telegram_government_v3_balance_guard",app,bot,logger)
    _install_module("telegram_government_final_flow_v7",app,bot,logger)
    _install_module("telegram_request_code_image_flow_v4",app,bot,logger)
    _install_module("telegram_final_bugfixes_v1",app,bot,logger)
    try:
    ...
    # These two guards MUST be installed last: several legacy layers rebind the
    # dispatcher/keyboards after the main module list has finished.
    _install_module("telegram_partner_actions_final_guard",app,bot,logger)
    _install_module("telegram_persian_only_final_guard",app,bot,logger)
