...
def _install_features(app):
    ...
    F.install(app,B)
    import telegram_ui_policy as UI
    UI.install(B)
    ...
    try:
        import telegram_admin_entry as AE
        AE.install(app,B)
    ...
    R.install(app,B)
    import telegram_night_shift as N
    N.install(app,B)
...
def build():
    ...
    except Exception:log.exception("ui callback unavailable")
