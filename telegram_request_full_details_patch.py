"""Canonical complete request notification and admin request controls."""
import logging
...
def _keyboard(rid,lang="fa",include_view=False):
 try:
  import request_language_actions as L
  return L.admin_markup(rid,lang,include_view=include_view)
 except Exception:
...
def _full_text(B,rid):
 ...
 lang=_lang(B,rid)
 try:
  L=__import__("request_language_actions")
...
async def _bottom(update,context,B,rid):
 ...
 if text is None:
  await update.callback_query.answer("Request not found",show_alert=True); raise ApplicationHandlerStop
 lang=_lang(B,rid); q=update.callback_query
 await q.message.reply_text(text,reply_markup=_keyboard(rid,lang,False))
 await _send_media(context.bot,q.from_user.id,files,rid,lang)
 await q.answer("📤 انتقال به آخر چت")
...
def install(app,B):
 ...
 app.add_handler(CallbackQueryHandler(callback,pattern=r"^req:bottom:\d+$"),group=-110)
 B._request_full_details_patch=True
 return True

def finalize(B):
 """Make request notifications complete without sending a duplicate summary."""
