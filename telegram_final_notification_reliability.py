...
def install(app, B):
    if getattr(B, "_final_notification_reliability", False): return True
    async def _send_with_retry(bot, method, chat_id, **kwargs):
        for attempt in range(3):
            try:
                await method(chat_id=chat_id, **kwargs); return True
            except Exception:
                if attempt == 2: log.exception("notification delivery failed chat=%s", chat_id)
                else: await asyncio.sleep(0.7 * (attempt + 1))
        return False
    async def notify_admins(application, message, request_id=None, inline=None, files=None):
        ...
        if request_id:
            try:
                rows = B.db.conn.execute("SELECT field_key,file_id FROM request_answers WHERE request_id=? AND file_id!='' ORDER BY id", (int(request_id),)).fetchall()
                known = {str(x.get("file_id") or "").strip() if isinstance(x,dict) else str(x or "").strip() for x in attachment_items}
                for row in rows:
                    fid = str(row["file_id"] or "").strip()
                    if fid and fid not in known:
                        attachment_items.append({"type":"photo","file_id":fid,"field_key":row["field_key"]}); known.add(fid)
            except Exception: log.exception("request attachment lookup failed: request=%s", request_id)
        tracking = str(request_id or "")
        if request_id:
            try:
                row = B.db.conn.execute("SELECT tracking_code FROM requests WHERE id=?", (int(request_id),)).fetchone()
                if row and row["tracking_code"]: tracking = str(row["tracking_code"])
            except Exception: pass
        all_ok = True; bot = application.bot
        for aid in admins:
            try: admin_id = int(aid)
            except Exception: all_ok = False; continue
            if not await _send_with_retry(bot, bot.send_message, admin_id, text=str(message), reply_markup=markup): all_ok = False
            for item in attachment_items:
                if not item: continue
                if isinstance(item, dict): fid=str(item.get("file_id") or "").strip(); kind=str(item.get("type") or "photo").lower(); field=str(item.get("field_key") or "فایل درخواست")
                else: fid=str(item).strip(); kind="photo"; field="فایل درخواست"
                if not fid: continue
                caption=f"📎 {field} | درخواست {tracking}"
                sent = await _send_with_retry(bot, bot.send_document if kind == "document" else bot.send_photo, admin_id, **({"document":fid,"caption":caption} if kind == "document" else {"photo":fid,"caption":caption}))
