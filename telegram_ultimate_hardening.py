...
def _fake(update, text):
    ...
    src = q.message
    class Msg:
        def __init__(self, original, value): self._original = original; self.text = value
...
def _clean(s): return str(s or "").strip()
...
def _ik_label(data, q):
    ...
    try:
        for row in getattr(q.message.reply_markup, "inline_keyboard", []) or []:
            for b in row:
                if str(getattr(b, "callback_data", "")) == data: return _clean(getattr(b, "text", ""))
    except Exception: pass
    return ""
...
async def _callback(update, context, B):
    q = update.callback_query; data = _clean(q.data)
    if not (data.startswith("ik:") or data.startswith("ui:")): return
    await q.answer(); label = _ik_label(data, q) if data.startswith("ik:") else data[3:]
    if not label:
    ...
    if label in {B.CANCEL, "❌ لغو", "Cancel", "❌ Cancel", "إلغاء", "❌ إلغاء"}:
        await B.cancel(fake, context); raise ApplicationHandlerStop
    try:
        ...
        await q.message.reply_text("❌ اجرای این گزینه با خطا مواجه شد. لطفاً دوباره تلاش کنید."); raise ApplicationHandlerStop


async def _ticket_reply(update, context, B):
    q = update.callback_query; data = _clean(q.data)
    if not data.startswith("ticket:reply:"): return
    await q.answer()
    try: pid = int(data.rsplit(":", 1)[1])
    except Exception:
        await q.message.reply_text("❌ شناسه تیکت نامعتبر است."); raise ApplicationHandlerStop
    uid = q.from_user.id; st = B.S.setdefault(uid, {})
    if B.admin(uid): st["mode"] = "ticket_admin_reply"; st["ticket_partner_id"] = pid
    else:
        if st.get("partner_id") and int(st["partner_id"]) != pid:
            await q.message.reply_text("❌ این تیکت متعلق به شما نیست."); raise ApplicationHandlerStop
        st["partner_id"] = pid; st["mode"] = "ticket_partner_reply"
...
async def _topup_amount(update, context, B):
    ...
    if st.get("mode") != "topup_amount": return
    ...
    if not p:
        st["mode"] = None; return await update.message.reply_text("❌ حساب همکار پیدا نشد.", reply_markup=B.main(update.effective_user.id))
...
async def _gov_text(update, context, B, old):
    st = B.S.setdefault(update.effective_user.id, {}); mode = st.get("mode"); text = _clean(update.message.text)
    digits = _digits(text).replace(" ", "").replace("-", ""); lang = st.get("lang", "fa")
    if mode == "gov_special":
        if len(digits) < 3: return await update.message.reply_text("❌ شناسه اختصاصی را صحیح وارد کنید.", reply_markup=B.cancel_kb(lang))
        st["gov_special"] = digits; st.pop("gov_family_code", None); st["mode"] = "gov_identity"
    ...
    if mode in {"gov_identity", "gov_family", "gov_passport"}:
        if len(digits) < 3: return await update.message.reply_text("❌ شماره مدرک را صحیح وارد کنید.", reply_markup=B.cancel_kb(lang))
        if st.get("gov_doc_type") == "passport": st["gov_passport"] = text
        else: st["booklet_number"] = digits
        st.pop("gov_family_code", None); st["mode"] = "gov_postal"
        return await update.message.reply_text("📮 حالا کد پستی ۱۰ رقمی منزل مشترک را وارد کنید:", reply_markup=B.cancel_kb(lang))
    if mode == "gov_postal":
        if not re.fullmatch(r"\d{10}", digits): return await update.message.reply_text("❌ کد پستی منزل باید دقیقاً ۱۰ رقم باشد.", reply_markup=B.cancel_kb(lang))
        st["postal_code"] = digits; st["mode"] = "gov_photo"
...
async def _gov_media(update, context, B, old):
    st = B.S.setdefault(update.effective_user.id, {})
    if st.get("mode") != "gov_photo" or not st.get("partner_id"): return await old(update, context)
    ...
    pid = int(st["partner_id"]); amount = int(B.db.setting("price_government", "500000") or 500000); conn = B.db.conn
    try:
        conn.commit(); conn.execute("BEGIN IMMEDIATE")
        p = conn.execute("SELECT * FROM partners WHERE id=? AND active=1", (pid,)).fetchone(); balance = int(p["balance"] or 0) if p else 0
        if not p or balance < amount:
            conn.rollback(); return await msg.reply_text(f"❌ اعتبار کافی نیست.\n💰 هزینه خدمت: {amount:,} تومان\n💳 اعتبار فعلی: {balance:,} تومان", reply_markup=B.partner_kb(st.get("lang", "fa")))
        code = "NYM-" + secrets.token_hex(4).upper(); t = B.now()
        cur = conn.execute("INSERT INTO requests(tracking_code,user_id,service_key,platform,status,amount,payment_status,payment_method,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)", (code,pid,"government","telegram","submitted",amount,"paid","partner_balance",t,t)); rid = cur.lastrowid
        ...
        else: fields.append(("booklet_number",st.get("booklet_number","")))
        ...
        if cur.rowcount != 1: raise RuntimeError("partner balance changed during charge")
        conn.commit(); left = balance - amount
    except Exception:
        try: conn.rollback()
        except Exception: pass
    ...
    try: await B.notify_admins(context.application, text, rid)
    except Exception: log.exception("government admin notification failed")
    st["mode"] = None; st.pop("gov_family_code", None)
    return await msg.reply_text(f"✅ درخواست ثبت شد.\n🎫 کد پیگیری: {code}\n💰 مبلغ کسرشده: {amount:,} تومان\n💳 اعتبار باقی‌مانده: {left:,} تومان", reply_markup=B.partner_kb(st.get("lang", "fa")))


def install(app, B):
    if getattr(B, "_telegram_ultimate_hardening", False): return
    try:
        import final_safety_patch
        final_safety_patch.install(app)
    except Exception: log.exception("final_safety_patch install failed")
    old_topup = getattr(B, "topup", None)
    if old_topup: B.topup = lambda update, context: _topup_entry(update, context, B)
    old_service_text = getattr(B, "service_text", None)
    if old_service_text:
        async def service_text(update, context):
            if B.S.get(update.effective_user.id, {}).get("mode") == "topup_amount": return await _topup_amount(update, context, B)
            return await old_service_text(update, context)
        B.service_text = service_text
    old_gov_text = getattr(B, "gov_text", None)
    if old_gov_text:
        async def gov_text(update, context): return await _gov_text(update, context, B, old_gov_text)
        B.gov_text = gov_text
    old_media = getattr(B, "media", None)
    if old_media:
        async def media(update, context): return await _gov_media(update, context, B, old_media)
        B.media = media
