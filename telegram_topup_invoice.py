...
def install(B):
    if getattr(B, "_topup_invoice_ui", False):
        return
    old_router = B.router
    ...
    async def router(update, context):
        ...
        if not raw.isdigit() or int(raw) <= 0:
            return await update.message.reply_text("❌ مبلغ نامعتبر است. مثال: 500000", reply_markup=B.cancel_kb(st.get("lang", "fa")))
        pid = st.get("partner_id")
        p = B.db.conn.execute("SELECT * FROM partners WHERE id=?", (pid,)).fetchone()
        if not p:
            return await update.message.reply_text("❌ حساب همکار پیدا نشد.", reply_markup=B.partner_kb(st.get("lang", "fa")))
        amount = int(raw)
        st["mode"] = "topup_receipt"
        st["topup_amount"] = amount
        card, owner = _card(B)
        text = (
            "🧾 <b>فاکتور شارژ حساب همکار</b>\n"
            "━━━━━━━━━━━━━━━━━━\n"
            f"👤 همکار: <b>{p['name']}</b>\n"
    ...
    async def callback(update, context):
        ...
        data = str(q.data or "")
        if not data.startswith("topupui:"):
            return
        await q.answer()
        uid = q.from_user.id
        st = B.S.setdefault(uid, {})
    ...
    # Callback is added only for the invoice's own two controls.
    if hasattr(B, "_topup_invoice_app"):
        return
    B._topup_invoice_app = True
