...
This layer is installed last and registered in a much earlier handler group so
legacy ui2 callback wrappers cannot intercept partner buttons first.
"""
import inspect
import logging
...
async def _safe_cancel(update, context, B):
    ...
    old_lang = st.get("lang", "fa")
    partner_id = st.get("partner_id")
    # Clear active service/chat continuation state, but preserve identity data.
    ...
    if st.get("partner_id") and st.get("partner_active"):
        try:
            p = B.db.conn.execute("SELECT id,name,phone,balance FROM partners WHERE id=? AND active=1 LIMIT 1", (int(st["partner_id"]),)).fetchone()
        except Exception:
            p = None
        if p:
            return await q.message.reply_text(
                f"👥 پنل همکاران\n👤 {p['name'] or '-'}\n📱 {p['phone'] or '-'}\n💰 اعتبار: {int(p['balance'] or 0):,} تومان",
                reply_markup=B.partner_kb(old_lang),
            )
...
async def _dispatch(update, context, B, label):
    ...
    if label == CANCEL:
        return await _safe_cancel(update, context, B)
    ...
    if label == IRANCELL:
        try:
            ...
            await q.message.reply_text("❌ شروع خدمت ایرانسل با خطا مواجه شد؛ موجودی و پنل شما حفظ شد.", reply_markup=B.partner_kb(st.get("lang", "fa")))
        return
    ...
    if label == MANAGEMENT:
        try:
            ...
            log.exception("management start failed")
            await q.message.reply_text("❌ ارتباط با مدیریت فعلاً با خطا مواجه شد؛ پنل شما حفظ شد.", reply_markup=B.partner_kb(st.get("lang", "fa")))
        return
    ...
    if label == SIM_SERVICE:
        try:
            ...
            log.exception("SIM service start failed")
            await q.message.reply_text("❌ خدمات سیم کارت فعلاً با خطا مواجه شد؛ پنل شما حفظ شد.", reply_markup=B.partner_kb(st.get("lang", "fa")))
        return
    ...
    if label == LOGOUT:
        ...
        await q.message.reply_text("✅ از پنل همکاران خارج شدید.", reply_markup=B.main(uid))
        return
    ...
    if fn_name == "ticket":
        try:
            ...
            await q.message.reply_text("❌ ثبت تیکت فعلاً با خطا مواجه شد؛ پنل شما حفظ شد.", reply_markup=B.partner_kb(st.get("lang", "fa")))
        return
    ...
    if fn_name:
        ...
        if fn:
            try:
                ...
                log.exception("service start failed label=%r", label)
                await q.message.reply_text("❌ اجرای خدمت فعلاً با خطا مواجه شد؛ مرحله شما حفظ شد.", reply_markup=B.partner_kb(st.get("lang", "fa")))
                return

    # Fallback to the canonical v29 dispatcher, never directly into the legacy
    # raw ui2 dispatcher which contains old sync/async assumptions.
    try:
        from telegram_partner_final_router_v29 import final_dispatch
        result = final_dispatch(update, context, B, label)
        if _awaitable(result):
            await result
        return
    except Exception:
        log.exception("v30 fallback failed label=%r", label)
        await q.message.reply_text("⛔ این گزینه فعلاً اجرا نشد؛ پنل همکاران شما حفظ شد.", reply_markup=B.partner_kb(st.get("lang", "fa")))


def install(app, B):
    if getattr(B, "_absolute_callback_v30", False):
        return
    ...
    async def callback(update, context):
        ...
        if not label:
            try:
                for rr in getattr(q.message.reply_markup, "inline_keyboard", []) or []:
                    for b in rr or []:
                        if getattr(b, "callback_data", None) == q.data:
                ...
                pass
        if not label:
            await q.answer("این دکمه دیگر معتبر نیست؛ پنل همکاران را دوباره باز کنید.", show_alert=True)
            raise ApplicationHandlerStop
        try:
            await q.answer()
        except Exception:
        ...
        try:
            ...
            log.exception("v30 callback failed label=%r", label)
            if st.get("partner_id") and st.get("partner_active") and not st.get("partner_logged_out"):
                await q.message.reply_text("❌ خطای موقت؛ پنل همکاران شما حفظ شد.", reply_markup=B.partner_kb(st.get("lang", "fa")))
            else:
                await q.message.reply_text("❌ این گزینه فعلاً اجرا نشد. لطفاً دوباره پنل را باز کنید.", reply_markup=B.main(q.from_user.id))
        raise ApplicationHandlerStop

    app.add_handler(CallbackQueryHandler(callback, pattern=r"^ui2:"), group=-2000000)
    B._absolute_callback_v30 = True
    log.info("ABSOLUTE Telegram ui2 callback hardening v30 installed")
