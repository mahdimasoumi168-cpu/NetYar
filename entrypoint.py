...
import server
NETYAR_TELEGRAM_BUILD="2026-09-17-admin-canonical-v4"
...
def _install_telegram_layers(app,bot,logger):
    ...
    _install_module("telegram_night_shift_v2",app,bot,logger)
    _install_module("telegram_admin_cleanup_and_night_switch_v1",app,bot,logger)
    # Absolute final owner: installed last and intercepts admin entry before
    # every legacy text/callback handler, eliminating stale-menu swallowing.
    _install_module("telegram_canonical_admin_final",app,bot,logger)
    logger.info("FINAL Telegram admin menu owner installed: absolute canonical admin + night controls")
def main():
