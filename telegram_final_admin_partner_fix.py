...
def install(app,B):
    ...
    B.db.conn.execute("""CREATE TABLE IF NOT EXISTS partner_service_prices(partner_id INTEGER NOT NULL,service_key TEXT NOT NULL,price INTEGER NOT NULL,updated_at TEXT NOT NULL,PRIMARY KEY(partner_id,service_key))"""); B.db.conn.commit()
    try:
        import telegram_admin_plus as A
        ...
        if not getattr(A,"_hard_logout_menu_patch",False):
            def menu_with_final_controls():
                ...
                rows.append([InlineKeyboardButton("📈 قیمت اختصاصی همکار — تک‌تک خدمات",callback_data="adm:partner_price_adjust_final")])
    ...
    async def text(update,context):
        ...
        if mode=="fpp_partner":
            ...
            if not services: st["mode"]=None; await update.message.reply_text("❌ هیچ خدمتی تعریف نشده است.",reply_markup=_admin_menu(B)); raise ApplicationHandlerStop
            st.update({"fpp_partner_id":int(p["id"]),"fpp_service_index":0,"fpp_prices":{},"mode":"fpp_price"}); r=services[0]
        ...
        B.db.conn.execute("INSERT OR REPLACE INTO partner_service_prices VALUES(?,?,?,?)",(pid,r["key"],value,_now())); B.db.conn.commit(); idx+=1
        if idx<len(services):
        ...
        st["mode"]=None; await update.message.reply_text("✅ قیمت اختصاصی همه خدمات این همکار، تک‌تک ثبت شد.",reply_markup=_admin_menu(B)); raise ApplicationHandlerStop
