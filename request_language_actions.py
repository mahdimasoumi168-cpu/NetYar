...
LABELS={"fa":{"new":"🆕 درخواست جدید","details":"📋 جزئیات کامل درخواست","tracking":"🎫 کد پیگیری","service":"🧾 خدمت","status":"📌 وضعیت","amount":"💰 مبلغ","payment":"💳 وضعیت پرداخت","method":"💵 روش پرداخت","time":"🕐 زمان ثبت","info":"📋 اطلاعات ثبت‌شده","files":"📎 فایل پیوست دارد","none":"• اطلاعات تکمیلی ثبت نشده است.","ask":"📨 درخواست کد از همکار","resend":"📤 انتقال به آخر چت","view":"🔎 مشاهده اطلاعات کامل","review":"⏳ در حال بررسی","done":"✅ انجام شد","reject":"❌ رد درخواست","reply":"✉️ پاسخ به مشترک","back":"⬅️ بازگشت به پنل مدیریت"},"en":{"new":"🆕 New Request","details":"📋 Full Request Details","tracking":"🎫 Tracking Code","service":"🧾 Service","status":"📌 Status","amount":"💰 Amount","payment":"💳 Payment Status","method":"💵 Payment Method","time":"🕐 Created At","info":"📋 Submitted Information","files":"📎 Attachment included","none":"• No additional information was submitted.","ask":"📨 Request Code from Partner","resend":"📤 Move to End of Chat","view":"🔎 View Full Information","review":"⏳ Under Review","done":"✅ Completed","reject":"❌ Reject Request","reply":"✉️ Reply to Customer","back":"⬅️ Back to Admin Panel"},"ar":{"new":"🆕 طلب جديد","details":"📋 تفاصيل الطلب كاملة","tracking":"🎫 رمز التتبع","service":"🧾 الخدمة","status":"📌 الحالة","amount":"💳 المبلغ","payment":"💳 حالة الدفع","method":"💵 طريقة الدفع","time":"🕐 وقت التسجيل","info":"📋 المعلومات المسجلة","files":"📎 يوجد ملف مرفق","none":"• لم يتم تسجيل معلومات إضافية.","ask":"📨 طلب الرمز من الشريك","resend":"📤 نقل إلى آخر المحادثة","view":"🔎 عرض المعلومات الكاملة","review":"⏳ قيد المراجعة","done":"✅ تم الإنجاز","reject":"❌ رفض الطلب","reply":"✉️ الرد على المشترك","back":"⬅️ العودة إلى لوحة الإدارة"}}
...
def _language_from_states(owner):
    try:
        ...
        for st in getattr(B,"S",{}).values():
            if st.get("lang") in LABELS and (str(st.get("partner_id"))==str(owner) or str(st.get("user_id"))==str(owner)):return normalize_lang(st.get("lang"))
...
def _telegram_chat_from_states(owner):
    try:
        ...
        r=B.db.conn.execute("SELECT external_id FROM users WHERE id=? AND platform='telegram' LIMIT 1",(owner,)).fetchone()
        if r and str(r["external_id"]).strip():return str(r["external_id"]).strip()
    except Exception:pass
    return ""
def _patch_create_request(db):
    ...
    def create_request(user_id,service_key,platform,amount,*args,**kwargs):
        ...
        try:
            rid=int(result[0]);db.conn.execute("UPDATE requests SET language=? WHERE id=?",(_language_from_states(user_id),rid))
...
def request_language(db,rid):
    try:
        r=db.conn.execute("SELECT language FROM requests WHERE id=?",(rid,)).fetchone();return normalize_lang(r["language"] if r else "fa")
