...
def call(m,p=None):return api(m,p)
def lang_menu():return [[('1','🇮🇷 فارسی'),('2','🇬🇧 English'),('3','🇸🇦 العربية')]]
...
def handle(u,c,x,m=None):
 if m is None:m={'file':{'file_id':x}} if S.get(str(u),{}).get('step') in {'fidaid','tr','files','gid'} and x else {}
 ...
 if st=='language':
  ...
  if not z:return send(c,T['fa']['lang'],lang_menu())
 ...
 if st=='pw':
  ...
  s['partner_id']=p['id'];s['step']='pm';db.set_setting(f'partner_chat_{p["id"]}',c);return send(c,'👥 پنل همکاران\n'+tr(u,'bal',n=int(p['balance'])),partner_kb())
 if st=='pm':
...
def process(u):
 ...
 x=txt(m)
 if x=='/start':S[str(sid)]={'lang':'fa','step':'language'};return send(c,T['fa']['lang'],lang_menu())
