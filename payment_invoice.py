...
def _card(B=None):
    ...
    if not value and B is not None:
        try:value = B.db.setting("payment_card", "").strip()
...
def _owner(B=None):
    ...
    if not value and B is not None:
        try:value = B.db.setting("payment_card_owner", "").strip()
        except Exception:value = ""
    return value
...
def invoice_text(title, amount, tracking_code=None, B=None):
    card,owner=_card(B),_owner(B)
    lines=[f"🧾 <b>{title}</b>","━━━━━━━━━━━━━━━━━━",f"💰 مبلغ قابل پرداخت: <b>{int(amount):,} تومان</b>"]
    if tracking_code:lines.append(f"🎫 کد پیگیری: <code>{tracking_code}</code>")
    lines += ["━━━━━━━━━━━━━━━━━━","💳 روش پرداخت: کارت‌به‌کارت"]
    lines.append(f"💳 شماره کارت: <code>{card}</code>" if card else "💳 شماره کارت در تنظیمات مدیریت ثبت نشده است.")
    if owner:lines.append(f"👤 به نام: <b>{owner}</b>")
    lines += ["","📸 پس از واریز، تصویر رسید پرداخت را همینجا ارسال کنید.","⚠️ انجام خدمت فقط پس از بررسی و تأیید پرداخت توسط مدیریت انجام می‌شود."]
    return "\n".join(lines)


def invoice_markup(B=None, tracking_code=None, amount=None, title=None):
    # No dead payment/link button: the receipt handler listens for the next photo/document.
