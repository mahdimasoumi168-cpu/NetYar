...
async def callback_data(data):
 ...
 if data.get('ResCod') not in {'0','00'}:set_result(order_id,'failed',data.get('Message','پرداخت ناموفق'));return HTMLResponse('<h3>❌ پرداخت ناموفق یا لغو شد.</h3>')
 ...
 try:
  ...
  set_result(order_id,'paid',result.get('Message','پرداخت تأیید شد'),trans_no=result.get('TransNo'),ref_no=result.get('RefNo'),trace_no=result.get('TraceNo'));return HTMLResponse('<h2>✅ پرداخت با موفقیت تأیید شد.</h2><p>اکنون به ربات برگردید.</p>')
 except Exception as e:set_result(order_id,'error',str(e)[:500]);return HTMLResponse('<h3>❌ خطا در تأیید تراکنش.</h3>',status_code=502)
...
def start_child(cmd,name):
 env=os.environ.copy();env['TELEGRAM_WEBHOOK_URL']=''
 p=subprocess.Popen([sys.executable,cmd],env=env);children.append((p,name));return p
def monitor():
 while True:
  time.sleep(5)
  for p,name in list(children):
   if p.poll() is not None:
    ...
    try:start_child('telegram_runtime.py' if name=='telegram' else 'rubika_runtime.py',name)
    except Exception:pass
def main():
 init_db()
 start_child('telegram_runtime.py','telegram')
 start_child('rubika_runtime.py','rubika')
