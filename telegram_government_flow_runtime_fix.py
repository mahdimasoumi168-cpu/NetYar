...
def install(app, B):
    ...
    async def text(update, context):
        ...
        if mode == "govv2_dob": st["gov_dob"] = t; st["mode"] = "govv2_unique"; await update.message.reply_text("🆔 شناسه یکتای مشترک را وارد کنید:", reply_markup=cancel); raise ApplicationHandlerStop
        ...
        if mode == "govv2_special":
            if not re.fullmatch(r"1\d{11}", d): await update.message.reply_text("❌ شناسه اختصاصی باید ۱۲ رقم و با ۱ شروع شود.", reply_markup=cancel)
                ...
                if typ == "card": st["mode"] = "govv2_family"; await update.message.reply_text("👨‍👩‍👧‍👦 کد خانوار مشترک را وارد کنید (فقط برای کارت آمایش):", reply_markup=cancel)
                    ...
                    st["mode"] = "govv2_identity_number"; prompt = ("🛂 شماره گذرنامه مشترک را وارد کنید:" if typ == "passport" else "📗 شماره دفترچه اقامت مشترک را وارد کنید:" if typ == "residence_booklet" else "🪪 شماره کارت موقت مشترک را وارد کنید:"); await update.message.reply_text(prompt, reply_markup=cancel)
            raise ApplicationHandlerStop
        ...
        if mode == "govv2_postal":
            if not re.fullmatch(r"\d{10}", d): await update.message.reply_text("❌ کد پستی باید دقیقاً ۱۰ رقم باشد.", reply_markup=cancel)
                ...
                st["gov_postal"] = d
                if st.get("gov_doc_type") == "passport": st["mode"] = "govv2_passport_photo1"; await update.message.reply_text("📸 ۱/۳ — لطفاً عکس صفحه اول پاسپورت مشترک را ارسال کنید:", reply_markup=cancel)
                else: st["mode"] = "govv2_photo"; await update.message.reply_text("📸 حالا تصویر مدرک مشترک را ارسال کنید:", reply_markup=cancel)
            raise ApplicationHandlerStop
    ...
    async def media(update, context):
        ...
        msg=update.message; fid,kind=_file_id(msg)
        if not fid: await msg.reply_text("❌ لطفاً عکس یا فایل مدرک را ارسال کنید.",reply_markup=cancel); raise ApplicationHandlerStop
        if mode=="govv2_passport_photo1": st["gov_passport_photo1"]=fid; st["mode"]="govv2_passport_photo2"; await msg.reply_text("📸 ۲/۳ — لطفاً عکس صفحه تمدید روادید را ارسال کنید:",reply_markup=cancel); raise ApplicationHandlerStop
        if mode=="govv2_passport_photo2": st["gov_passport_photo2"]=fid; st["mode"]="govv2_passport_photo3"; await msg.reply_text("📸 ۳/۳ — لطفاً عکس تمدید گذرنامه را ارسال کنید:",reply_markup=cancel); raise ApplicationHandlerStop
