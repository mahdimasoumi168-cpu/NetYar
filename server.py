    ...
    while True:
        ...
        for i,(p,name) in enumerate(list(children)):
            if p.poll() is not None:
                ...
                try: children.append((subprocess.Popen([sys.executable,"bot.py" if name=="telegram" else "rubika_bot.py"],env={**os.environ,"TELEGRAM_WEBHOOK_URL":""}),name))
                except Exception: pass

def main():
    init_db()
    start_child("bot.py","telegram")
    start_child("rubika_bot.py","rubika")
