"""Tehran 07:00-19:00 business-hours gate with configurable night partners."""
from datetime import datetime,time
from zoneinfo import ZoneInfo
from telegram import InlineKeyboardMarkup,InlineKeyboardButton
from telegram.ext import CallbackQueryHandler,MessageHandler,ApplicationHandlerStop,filters
TZ=ZoneInfo("Asia/Tehran");OPEN=time(7,0);CLOSE=time(19,0);PREFIX="night_worker:"
...
def closed():return ("⏰ ربات در حال حاضر خارج از ساعت کاری است.\n\n🕖 ساعت کاری: ۷ صبح تا ۷ شب به وقت تهران\n🌙 ساعت تعطیلی: ۷ شب تا ۷ صبح\n\nلطفاً در ساعت کاری مراجعه کنید.")
def markup():return InlineKeyboardMarkup([[InlineKeyboardButton("🔄 شروع مجدد",callback_data="night2:restart")]])
...
def install(app,B):
    ...
    def menu():
        m=old_menu();rows=[list(r) for r in m.inline_keyboard];rows.insert(-1,[InlineKeyboardButton("🌙 همکاران شب‌کار",callback_data="night2:menu")]);return InlineKeyboardMarkup(rows)
    A._admin_menu=menu
    async def cb(update,context):
        ...
        uid=q.from_user.id
        if d[1]=="restart":
            await q.answer();return await q.message.reply_text(closed(),reply_markup=markup())
        if not B.admin(uid):await q.answer("دسترسی ندارید",show_alert=True);return
