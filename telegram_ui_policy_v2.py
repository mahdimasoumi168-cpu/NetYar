...
def restart_keyboard(): return ReplyKeyboardMarkup([[RESTART]],resize_keyboard=True,one_time_keyboard=False,is_persistent=True)
...
def _resolve_partner(B, uid):
    ...
    try:
        if st.get("partner_id"):
            ...
            if row: st["partner_active"]=True; return True
        ...
        if row: st["partner_id"]=row["id"];st["partner_active"]=True;return True
...
def _fake(update,label):
    ...
    msg=MessageProxy(q.message,label); return SimpleNamespace(update_id=update.update_id,message=msg,effective_message=msg,effective_user=q.from_user,effective_chat=q.message.chat,callback_query=q)
...
async def _dispatch(update,context,B,label):
    ...
    if label=="🎫 تیکت به مدیریت":
        from telegram_business_features import _send_ticket_prompt; return await _send_ticket_prompt(fake,B)
...
def install(app,B):
    ...
    B.kb=lambda rows:inline(rows,B); B.restart_keyboard=restart_keyboard; B.main=lambda uid:inline(_main_rows(B,uid),B,uid); B.cancel_kb=lambda lang="fa":inline([[CANCEL]],B)
    def admin_menu():
        try:
    ...
    async def callback(update,context):
        ...
        token=str(q.data)[4:];row=B.db.conn.execute("SELECT user_id,label,lang,status FROM ui2_callbacks WHERE token=?",(token,)).fetchone()
        if not row:
