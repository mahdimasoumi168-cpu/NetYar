...
def api(method,payload=None):
    r=requests.post(f'{BASE}/{method}',json=payload or {},timeout=45); r.raise_for_status(); data=r.json()
...
def admin_flow(uid,chat,text):
    ...
    if text=='a_notify':send(chat,'🔔 اعلان خودکار درخواست‌ها فعال است؛ وضعیت‌ها از پنل مدیریت ثبت می‌شوند.',key(admin_menu()));return True
    ...
    if step=='bot_name':st['bot_name']=text;st['admin_step']='bot_token';send(chat,'🔐 API Token را ارسال کنید. در دیتابیس ذخیره نمی‌شود؛ فقط برای اعتبارسنجی لحظه‌ای استفاده می‌شود.');return True
    if step=='bot_token':
        try:
            ok,ref=add_bot(st['bot_platform'],st['bot_name'],text)
            st.pop('admin_step',None);send(chat,(f'✅ API معتبر بود و بات ثبت شد.\n🔑 Secret reference: {ref}\n⚠️ برای اجرای دائمی، توکن باید به عنوان Railway Secret با همین نام تنظیم شود.' if ok else f'⚠️ رکورد بات ثبت شد ولی API اعتبارسنجی نشد.\n🔑 Secret reference: {ref}'),key(admin_menu()));return True
            ...
            for u in updates:
                ...
                try:
                    ...
                    if text.lower() in ('/admin2025','/admin'):
                        if is_admin(uid):S.setdefault(uid,{})['admin']=True;send(chat,'🛠 پنل مدیریت پیشرفته فعال شد.',key(admin_menu()))
