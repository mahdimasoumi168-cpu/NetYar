...
def install(app,B):
 ...
 async def partner_for(r):
  # A request must always go back to the same partner/user who created it.
  # First resolve the request owner directly as a partner id.
  try:
   p=B.db.conn.execute("SELECT id,phone,name,active FROM partners WHERE id=? AND active=1",(r["user_id"],)).fetchone()
   if p:return p
  except Exception: pass
  # Then resolve the exact Telegram user/external id belonging to this request owner.
  try:
   ...
   if ext:
    for p in B.db.conn.execute("SELECT id,phone,name,active FROM partners WHERE active=1").fetchall():
     if ext in {str(B.db.setting(f"partner_chat_{p['id']}","")).strip(),str(B.db.setting(f"partner_chat_{p['phone']}","")).strip()}:
      return p
  except Exception: log.exception("exact request-owner partner resolution failed")
  # Persisted request mapping is the final exact-owner fallback.
  try:
   x=B.db.setting(f"request_partner_{r['id']}","").strip()
   if x.isdigit():
    return B.db.conn.execute("SELECT id,phone,name,active FROM partners WHERE id=? AND active=1",(int(x),)).fetchone()
  except Exception: pass
  return None
 ...
 async def ask(update,context):
  ...
  try:
   ...
   if not p:await q.answer("کاربری که این درخواست را ثبت کرده پیدا نشد",show_alert=True);raise ApplicationHandlerStop
   ...
   n+=1
   B.db.set_setting(f"partner_code_attempts_{rid}",str(n))
   ...
   for i in range(3):
    try:
     await context.bot.send_message(chat_id=chat,text=msg,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ لغو",callback_data=f"pc:x:{rid}:{p['id']}")]]));err=None;break
    except Exception as e:
     err=e;await asyncio.sleep(.7*(i+1))
   if err:raise err
   await q.answer("✅ درخواست کد برای کاربر ثبت‌کننده ارسال شد")
   await q.message.reply_text(f"✅ درخواست کد برای «{p['name'] or p['phone']}» ارسال شد.\n🎫 {r['tracking_code']}")
 ...
 async def reply(update,context):
  ...
  r=B.db.conn.execute("SELECT tracking_code,service_key FROM requests WHERE id=?",(rid,)).fetchone()
  p=B.db.conn.execute("SELECT name,phone FROM partners WHERE id=?",(int(st["partner_id"]),)).fetchone()
