...
# Rubika compatibility: support both documented ReceiveUpdate payload shapes
# and preserve the explicit IDs assigned to keypad buttons.
try:
    import json
    import rubika_v2 as _rb

    def _rb_message(update):
        if isinstance(update, dict) and isinstance(update.get("inline_message"), dict):
            return update["inline_message"]
    ...
    def _rb_text(update):
        ...
        if isinstance(aux, dict):
            return str(aux.get("button_id") or aux.get("button_text") or aux.get("text") or "").strip()
    ...
    _rb.text_of = _rb_text
    _rb.chat_of = _rb_chat
    _rb.user_of = _rb_user
    _rb.rows = _rb_rows
    _rb._NETYAR_RUBIKA_PATCHED = True
except Exception:
    pass
