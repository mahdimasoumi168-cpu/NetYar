...
def stop_all(*_):
    ...
    while time.time()<deadline and any(p.poll() is None for p in children.values()): time.sleep(0.2)
    for p in list(children.values()):
...
def spawn(name,script):
    env=os.environ.copy()
    # One and only one Telegram consumer: polling. Webhook is owned by no worker here.
...
if __name__=="__main__":
    ...
    spawn("telegram","bot.py"); spawn("rubika","rubika_bot.py")
    print("NetYar: Telegram + Rubika workers started",flush=True)
    try:
        while not stop_requested:
            for name,script in (("telegram","bot.py"),("rubika","rubika_bot.py")):
                p=children.get(name)
                if p is None or p.poll() is not None:
                    if stop_requested: break
                    code=None if p is None else p.returncode
                    print(f"NetYar: {name} worker stopped ({code}); restarting in 2s",flush=True)
                    time.sleep(2)
