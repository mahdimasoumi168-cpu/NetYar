...
import server
NETYAR_TELEGRAM_BUILD="2026-09-17-enamad-trust"
...
def _install_telegram_layers(app,bot,logger):
    ...
    # Final admin navigation owns the management keyboard. In particular it
    # guarantees that the admin-only "➕ افزودن همکار جدید" button is present
    # in the actual management panel and that its callback/text flow is live.
    _install_module("telegram_final_admin_navigation_v3",app,bot,logger)
    # Public eNAMAD trust option is intentionally installed last so later UI
    # layers cannot remove the button from the canonical public menu.
