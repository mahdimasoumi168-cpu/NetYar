...
def upsert_user(tg, **data):
    t=now(); r=db.execute("SELECT id FROM users WHERE id=?",(tg.id,)).fetchone()
    vals=(tg.username or "", data.get("full_name",tg.full_name or ""), data.get("phone",""), data.get("id_code",""), data.get("city",""))
    if r:
        db.execute("UPDATE users SET username=?,full_name=?,phone=?,id_code=?,city=?,updated_at=? WHERE id=?", vals+(t,tg.id))
    else:
        db.execute("INSERT INTO users(id,username,full_name,phone,id_code,city,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)", (tg.id,)+vals+(t,t))
    db.commit()
