 ...
 while True:
  ...
  for p,name in list(children):
   if p.poll() is not None:
    ...
    cmd='bot.py' if name=='telegram' else 'rubika_entry_v2.py'
    try: children.append((subprocess.Popen([sys.executable,cmd],env={**os.environ,'TELEGRAM_WEBHOOK_URL':''}),name))
    except Exception: pass
def main():
 init_db(); start_child('bot.py','telegram'); start_child('rubika_entry_v2.py','rubika'); threading.Thread(target=monitor,daemon=True).start()
